...
1) http://localhost:8000/item/1 - товар в рублях;
2) http://localhost:8000/pay_item/1 - sessionId для оплаты товара в рублях;

3) http://localhost:8000/item/4 - товар в евро
4) http://localhost:8000/pay_item/4 - sessionId для оплаты товара в евро;

5) http://localhost:8000/order/1 - успешно оплаченный заказ;
6) http://localhost:8000/pay_order/1 - ошибка при повторной попытке оплатить заказ;

7) http://localhost:8000/order/2 - заказ в рублях
8) http://localhost:8000/pay_order/2 - sessionId для оплаты заказа в рублях;

9) http://localhost:8000/pay_order/3 - sessionId для оплаты заказа со скидкой и налогом в рублях;

10) http://localhost:8000/order/4 - заказ с разной валютой;
11) http://localhost:8000/pay_order/4 - ошибка при оплате заказа с разной валютой;

12) http://localhost:8000/stripe/webhook/ - вебхук (только для POST запросов), вызываемый Stripe после завершения сессии оплаты.
